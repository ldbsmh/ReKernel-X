...
/* ==========================================================================
 * Table-driven parsers — flat & nested (O(n) routing to O(1) array access)
 * ========================================================================== */

template<size_t MAX_ATTR>
static inline void nla_parse(struct nlattr *(&tb)[MAX_ATTR], void *data, int len) {
    memset(tb, 0, sizeof(tb));
    if (!data || len <= 0) return;

    struct nlattr *nla;
    int rem;
    nla_for_each_attr(nla, data, len, rem) {
        int type = nla_type(nla);
        if (type > 0 && type < MAX_ATTR) {
            tb[type] = nla;
        }
    }
}
...
template<size_t MAX_ATTR>
static inline void nla_parse_nested(struct nlattr *(&tb)[MAX_ATTR], const struct nlattr *nested) {
    memset(tb, 0, sizeof(tb));
    if (!nested) return;

    struct nlattr *nla;
    int rem;
    nla_for_each_nested(nla, nested, rem) {
        int type = nla_type(nla);
        if (type > 0 && type < MAX_ATTR) {
            tb[type] = nla;
        }
 ...
 * (The kernel's nla_get_* assume nla_policy already validated lengths.)
 * ========================================================================== */
