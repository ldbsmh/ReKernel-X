 ...
 * File: rekernel_x_genl.c — Generic Netlink transport.
 */
...
static int rekernel_x_genl_monitor_net(struct sk_buff *skb, struct genl_info *info)
{
    ...
    rekernel_x_log_debug("addMonitorUid uid=%d\n", muid);
    net_uid_add(muid);
    return 0;
}
...
static int rekernel_x_genl_del_monitor_net(struct sk_buff *skb, struct genl_info *info)
{
    ...
    muid = (uid_t)nla_get_u32(info->attrs[REKERNEL_X_A_UID]);
    rekernel_x_log_debug("delMonitorNet uid=%d\n", muid);
    net_uid_del(muid);
    return 0;
}
...
int sendMessage(struct rekernel_x_event *event)
{
    ...
    if (!skb)
    {
        rekernel_x_log_err("genlmsg alloc failure!\n");
        return LINE_ERROR;
    }
    ...
    if (!msg_head)
    {
        rekernel_x_log_err("genlmsg_put failure!\n");
        nlmsg_free(skb);
        return LINE_ERROR;
    }
    ...
    case REKERNEL_X_EVT_BINDER:
    {
        ...
        if (!payload)
            goto nla_fail;
        if (nla_put_s32(skb, REKERNEL_X_A_BINDER_TYPE, b->binder_type) ||
            nla_put_s32(skb, REKERNEL_X_A_BINDER_ONEWAY, b->oneway) ||
            nla_put_s32(skb, REKERNEL_X_A_BINDER_FROM_PID, b->from_pid) ||
            nla_put_s32(skb, REKERNEL_X_A_BINDER_FROM_UID, b->from_uid) ||
            nla_put_s32(skb, REKERNEL_X_A_BINDER_TARGET_PID, b->target_pid) ||
            nla_put_s32(skb, REKERNEL_X_A_BINDER_TARGET_UID, b->target_uid) ||
    ...
    case REKERNEL_X_EVT_SIGNAL:
    {
        ...
        if (nla_put_s32(skb, REKERNEL_X_A_SIGNAL_SIGNAL, s->signal) ||
            ...
            nla_put_s32(skb, REKERNEL_X_A_SIGNAL_KILLER_UID, s->killer_uid) ||
            nla_put_s32(skb, REKERNEL_X_A_SIGNAL_DST_PID, s->dst_pid) ||
            nla_put_s32(skb, REKERNEL_X_A_SIGNAL_DST_UID, s->dst_uid))
            goto nla_fail;
        nla_nest_end(skb, payload);
        break;
    }
    case REKERNEL_X_EVT_NETWORK:
    {
        ...
        if (!payload)
            goto nla_fail;
        if (nla_put_s32(skb, REKERNEL_X_A_NETWORK_PROTO, n->proto) ||
            nla_put_s32(skb, REKERNEL_X_A_NETWORK_TARGET_UID, n->target_uid) ||
    ...
    if (rc && rc != -ESRCH)
    {
        rekernel_x_log_err("genlmsg_multicast failed, rc=%d\n", rc);
        return LINE_ERROR;
    }
...
nla_fail:
    ...
    nlmsg_free(skb);
    rekernel_x_log_err("sendMessage: nla_put failed\n");
    return LINE_ERROR;
}

int register_genl(void)
{
    rekernel_x_log_info("Trying to register Generic Netlink family......\n");

    if (genl_register_family(&rekernel_x_genl_family) != 0)
    {
        rekernel_x_log_err("Failed to register genl family!\n");
        return LINE_ERROR;
    }
    rekernel_x_genl_registered = true;

    rekernel_x_log_info("Registered genl family! ID: %d\n", rekernel_x_genl_family.id);
    return LINE_SUCCESS;
}
