 ...
 * File: rekernel_x_binder_kp.c — Kprobe hooks for Binder transaction filtering.
 */
...
static int __nocfi binder_proc_transaction_pre(struct kprobe* p, struct pt_regs* regs)
{
	...
	if (line_is_frozen(proc->tsk)) {
		...
		if (t_outdated) {
			...
			rekernel_x_log_debug("free_outdated txn %d supersedes %d\n", t->debug_id, t_outdated->debug_id);
...
void __nocfi register_binder_kp(void) {
	...
	if (rc != LINE_SUCCESS) {
		rekernel_x_log_err("register kallsyms_lookup_name kprobe failed, rc=%d (binder async-cleanup disabled)\n", rc);
		return;
	}
	...
	if (re_binder_transaction_buffer_release == NULL || re_binder_alloc_free_buf == NULL || re_binder_stats == NULL) {
		rekernel_x_log_err("resolve binder symbols failed (binder async-cleanup disabled)\n");
		return;
	}

	rc = register_kprobe(&kp_binder_proc_transaction);
	if (rc != LINE_SUCCESS) {
		rekernel_x_log_err("register binder_proc_transaction kprobe failed, rc=%d (binder async-cleanup disabled)\n", rc);
		return;
	}
