 ...
 * File: rekernel_x_signal.c — Signal trace hooks & frozen task mitigation.
 */
...
void line_signal(void *data, int sig, struct task_struct *killer, struct task_struct *dst)
{
	...
	if (line_is_frozen(dst) &&
			...
			|| sig == SIGABRT
			|| sig == SIGQUIT)) {
		rekernel_x_log_debug("Process Signal! signal=%d\n", sig);
...
int register_signal(void)
{
	...
	if (rc != LINE_SUCCESS) {
		rekernel_x_log_err("register_trace_android_vh_do_send_sig_info failed, rc=%d\n", rc);
		return rc;
	}
	re_signal_hook = true;
	return LINE_SUCCESS;
